...
export class DataTableComponent implements OnInit {
   ...
   ngOnInit() {
      this.dataSource$
         .filter(data => !!data)
         // .do(data => (this.tableRows) ? data : this.normalizeData(data))
         .do(data => this.normalizeData(data))
   ...
   private getSVG(activeStatus: boolean, ascSortStatus: boolean): string {
      ...
      if (activeStatus && ascSortStatus) {
         return 'main:sort-ascending';
      }
      if (activeStatus && !ascSortStatus) {
         return 'main:sort-descending';
      }
   }

   private normalizeData(data): Object[] {
